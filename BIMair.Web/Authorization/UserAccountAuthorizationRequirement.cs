...
namespace BIMair.Authorization
{
    ...
    public class ManageProjectAuthorizationHandler : AuthorizationHandler<UserAccountAuthorizationRequirement, string>
    {
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, UserAccountAuthorizationRequirement requirement, string targetUserId)
        {
            if (context.User == null ||
            ...
            if (context.User.HasClaim(ClaimConstants.Permission, ApplicationPermissions.ManageProjects) || GetIsSameUser(context.User, targetUserId))
                context.Succeed(requirement);
    ...
    public class ManageCustomerAuthorizationHandler : AuthorizationHandler<UserAccountAuthorizationRequirement, string>
    {
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, UserAccountAuthorizationRequirement requirement, string targetUserId)
        {
            if (context.User == null ||
            ...
            if (context.User.HasClaim(ClaimConstants.Permission, ApplicationPermissions.ManageCustomers) || GetIsSameUser(context.User, targetUserId))
                context.Succeed(requirement);
